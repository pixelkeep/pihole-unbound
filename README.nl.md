...
### Configuratie (Pi-hole v6)

In Pi-hole v6 werkt `/etc/dnsmasq.d/` niet standaard meer.
Gebruik de webinterface:

```
http://<pihole-ip>/admin
→ Settings → rechtsboven: Advanced
→ All Settings → Miscellaneous
→ misc.dnsmasq_lines
→ voer in: address=/.pixelkeep.nl/192.168.20.15
→ Save & Apply
```
...
```bash
sudo pihole-FTL --config misc.dnsmasq_lines '["address=/.pixelkeep.nl/192.168.20.15"]'
sudo systemctl restart pihole-FTL
```

Vervang `pixelkeep.nl` door jouw eigen domein en `192.168.20.15` door het IP
van jouw reverse proxy (Caddy, Nginx Proxy Manager, etc.).

### Valideren

```bash
dig nas.pixelkeep.nl @192.168.20.11
# Verwacht: 192.168.20.15

dig vault.pixelkeep.nl @192.168.20.11
# Verwacht: 192.168.20.15
```

### Instellingen op meerdere Pi-hole instanties

Herhaal de configuratie op elke Pi-hole instantie. Voor pixelkeep.nl:

```
Primary:    http://192.168.20.11/admin
Secondary:  http://192.168.20.12/admin
```

### Alternatief: misc.etc_dnsmasq_d inschakelen

Als je meer controle wil via losse configuratiebestanden:

```
Settings → Advanced → All Settings → Miscellaneous
→ misc.etc_dnsmasq_d: Enabled
→ Save & Apply
```
...
```bash
sudo nano /etc/dnsmasq.d/99-local-wildcard.conf
```

Inhoud:

```
address=/.pixelkeep.nl/192.168.20.15
```
...
**Let op:** misc.dnsmasq_lines is eenvoudiger voor een paar regels.
misc.etc_dnsmasq_d is beter als je meerdere complexe configuratiebestanden hebt.
...
DHCP DNS servers per VLAN (ingesteld op de UDM Pro):

    DNS Server 1:   192.168.20.11   (RPDO01 — primary)
    DNS Server 2:   192.168.20.12   (DEBPIH01 — secondary)

Uitzondering:

    Guest (VLAN 60):    1.1.1.1 / 1.0.0.1   (gasten krijgen geen interne DNS)
    Cameras (VLAN 50):  geen DHCP DNS        (statische config)
